...
def TGC_gradiant(y_TGC, y_SM, dev_scale, n_MC, SM_xsection, lumi):
  """ Calculate the gradient wrt. to the TGC at the SM point.
...
def add_derivative_plot(ax, TGC_name, xy_dict, n_bins, hist_range, dev_scale,
  ...
  TGC_grad = functools.partial(TGC_gradiant, dev_scale=dev_scale, n_MC=n_MC, 
                                   SM_xsection=SM_xsection, lumi=lumi)
...
def create_reweighting_plot(xy_dict, output_base, obs_name, obs_range, 
                            lumi, SM_xsection, dev_scale, cut_dev,
  ...
  for TGC_name in ["g1z", "ka", "la"]:
    ...
    ax_up.set_ylabel(r"$\frac{{d\sigma^{{bin}}}}{{d {}}} [\frac{{fb}}{{1}}]$".format(TGC_name_map[TGC_name]), fontsize=30)
...
def check_reweighting(root_file, tgc_config_path, tgc_point_path, output_dir,
  ...
  """
  rdf = ROOT.RDataFrame(tree_name, root_file).Range(0,1000).Filter("rescan_weights.weight1 > 0.01")
  ...
  for obs_name, obs_range in observables.items():
    ...
    create_reweighting_plot(xy_dict, output_base, obs_name, obs_range, lumi, 
                            SM_xsection.GetValue(), dev_scale, cut_dev)
